...
//        outputToConsole.printMessage();
        try (ObjectOutputStream output = new ObjectOutputStream(new FileOutputStream("C:\\work\\JavaRush\\files\\OutputToConsole.bin"))) {
            outputToConsole.writeExternal(output);
        } catch (IOException e) {
            e.printStackTrace();
        }

        OutputToConsole o1 = new OutputToConsole();
        try (ObjectInputStream input = new ObjectInputStream(new FileInputStream("C:\\work\\JavaRush\\files\\OutputToConsole.bin"))) {
            o1.readExternal(input);
        } catch (IOException e) {
            e.printStackTrace();
